...
>
> **Cập nhật Tháng 8/2026:** checklist bên dưới đã đối chiếu lại với code thật trong repo
> (đọc trực tiếp toàn bộ route API + component liên quan). Trước bản cập nhật này, checklist
> gần như để trống dù thực tế Phase 1–4 đã build xong từ lâu — sửa lại để các phiên AI/dev
> sau không hiểu nhầm là dự án còn ở giai đoạn đầu và đề xuất làm lại những phần đã có.
> Các mục còn `[ ]` là thật sự chưa làm hoặc chưa xác nhận được — không suy đoán.

### Phase 0 — Hạ tầng (làm 1 lần duy nhất)
- [x] Tạo database MySQL/MariaDB trên Hostinger Business Shared Hosting · lấy DATABASE_URL
- [x] Tạo NEXTAUTH_SECRET
- [x] Clone repo · cấu hình `.env.local` trên hosting (production đang chạy tại hub.citedu.vn)
- [x] `npx prisma db push` · `npm run db:seed` (đã có 3 tài khoản mẫu qua `prisma/seed.ts`)
- [x] Cấu hình SSL Let's Encrypt qua hPanel (Hostinger Business Shared Hosting — không dùng Nginx tự cấu hình, xem Phần 1)
- [x] Setup GitHub Actions deploy workflow (`.github/workflows/deploy.yml` đang chạy)
- [ ] Setup cronjob backup mysqldump qua hPanel Cron Jobs — **chưa xác nhận được từ code** (nằm ngoài repo, cần anh xác nhận trực tiếp trên hPanel)
- [x] Xác nhận: hub.citedu.vn hoạt động, đăng nhập được (production đang chạy thật)

### Phase 1 — MVP: Lớp học & Dòng tiền ⭐ QUAN TRỌNG NHẤT — ĐÃ XONG
- [x] Auth: đăng nhập · middleware phân quyền · redirect theo role (`middleware.ts`, `lib/auth.ts`, `lib/auth-helpers.ts`)
- [x] Admin: tạo lớp · tạo ClassSession · thêm học viên (nhập agreed_price) (`admin/classes`, `admin/classes/[id]/sessions`, `admin/classes/[id]/enrollments`)
- [x] CNL: xem lớp mình · danh sách học viên · thông tin học phí (`(cnl)/my-classes`)
- [x] Thu học phí: CNL tạo phiếu → Admin thấy badge → Admin duyệt + nhập paid_at (`api/payments`, `api/admin/payments/[id]/approve|reject`)
- [x] Dashboard Admin: tổng công nợ · badge phiếu chờ duyệt · lead cần gọi lại hôm nay (`(admin)/dashboard`)

### Phase 2 — CRM ⭐ Kéo lên sớm (bài toán rơi lead cấp bách) — ĐÃ XONG
- [x] Quản lý Contact: tạo · sửa · tìm kiếm · Smart Match SĐT (`api/contacts`, `normalizePhone()`)
- [x] Pipeline Lead: Kanban 4 cột (new/consulting/won/lost) (`LeadKanbanBoard.tsx`)
- [x] LeadNote: ghi mỗi lần liên hệ (method + result + next_followup_at)
- [x] Dashboard "Lead cần gọi lại hôm nay" (`api/admin/dashboard/followups`)
- [x] Import Google Sheets: smart match + báo cáo X mới / Y trùng / Z xung đột (`api/admin/contacts/import`, `ContactImportClient.tsx`)

### Phase 3 — Vận hành lớp học — ĐÃ XONG (+ 1 tính năng bổ sung ngoài roadmap)
- [x] Điểm danh 1 chạm trên điện thoại CNL (`AttendanceBoard.tsx`, `api/cnl/sessions/[id]/attendance`)
- [x] Form đánh giá cuối buổi (MagicLink · expires_at · 1-time use) (`api/cnl/sessions/[id]/evaluation-link`, `app/feedback/[token]`)
- [x] Quản lý bài giảng (Lesson content · xem theo lớp) — có thêm `LessonVersion` (lịch sử bất biến) và AI gợi ý nội dung qua Gemini (`api/admin/lessons/[id]/ai-suggest`) — **bổ sung ngoài roadmap ban đầu**
- [x] Hoàn tiền workflow (chặn Nhánh 2) (`api/refunds`, `api/admin/refunds/[id]/approve|reject` — chặn Coaching tại API)
- [x] Waitlist management (`api/admin/enrollments/[id]/activate`)
- [x] Bảo lưu học viên (suspended + suspended_reason + suspended_until) (`api/admin/enrollments/[id]/suspend|reactivate`)
- [x] Học viên vào lớp muộn (cảnh báo X/Y buổi đã học · Admin nhập agreed_price) (`joined_late`, `joined_at_session` trong `classes/[id]/enrollments`)
- [x] **Học bù (Makeup Session)** — tính năng không có trong roadmap gốc nhưng đã build đầy đủ (`api/makeup-sessions`, `MakeupRegistrationSheet.tsx`)

### Phase 4 — Báo cáo & Tiện ích — ĐÃ XONG (trừ 1 mục cần quyết định)
- [x] Báo cáo doanh thu theo tháng (theo paid_at) (`api/admin/reports/revenue`)
- [x] Báo cáo công nợ tổng · dự thu (enrolled nhưng chưa đủ tiền) (`api/admin/reports/debt`)
- [x] Lọc học viên đủ điều kiện lên cấp (`api/admin/reports/progression`)
- [ ] Xem xét AuditLog nếu quy mô đòi hỏi — vẫn là quyết định mở, chưa cần tới lúc này (audit trail nhẹ qua `created_by_id`/`deleted_by_id`/`updated_at` vẫn đang đủ dùng)

### Phase 5 — AI & Automation — CHƯA LÀM
- [ ] Gemini Pro Chatbot truy xuất dữ liệu — chưa có; hiện Gemini mới chỉ dùng cho AI gợi ý nội dung bài giảng (Phase 3), không phải chatbot truy vấn dữ liệu
- [ ] Zalo cá nhân webhook thông báo phiếu thu mới cho Admin — chưa có, Admin vẫn chỉ thấy badge trong app khi mở Dashboard
